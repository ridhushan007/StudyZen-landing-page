...
const Sidebar = ({ children }) => (
  <div className="w-64 h-[calc(100vh-8rem)] bg-indigo-800 text-white p-4 flex flex-col rounded-l-lg">
...
export default function StudyZenBrowserAnimation() {
  const cursorControls = useAnimation()
  const contentControls = useAnimation()

  const runAnimation = async () => {
    // Move cursor to dashboard item
    await cursorControls.start({ x: 100, y: 160, transition: { duration: 1 } })
